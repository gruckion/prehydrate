...
<Check>Arrow functions work</Check>
<Check>Named functions work</Check>
<Check>Methods work (as standalone functions)</Check>
...
<Check>`formatTime` - valid</Check>
<Check>`_helper` - valid</Check>
<Check>`$config` - valid</Check>
<Warning>`format-time` - invalid (hyphens not allowed)</Warning>
<Warning>`123abc` - invalid (can't start with number)</Warning>
