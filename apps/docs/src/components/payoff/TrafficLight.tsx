...
export type LightState = 'red' | 'orange' | 'green' | 'none' | 'all';
...
const LightBulb = ({
  ...
  return (
    <g style={{ filter: glowFilter }}>
      ...
      {isOn && (
        <circle cx={50} cy={cy} r={12} fill={color} opacity={0.9} />
      )}
      <circle
        cx={44}
        cy={cy - 6}
...
export const TrafficLight = ({
  ...
  const isRedOn = activeLight === 'red' || activeLight === 'all';
  const isOrangeOn = activeLight === 'orange' || activeLight === 'all';
  const isGreenOn = activeLight === 'green' || activeLight === 'all';
